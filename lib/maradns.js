...
import dnsu from './util.js'
import mara from '../dist/mara.js'

const rr = new RR.AAAA(null)

export const zoneOpts = {}

export default { zoneOpts, parseZoneFile, expandShortcuts }
...
export async function parseZoneFile (str) {
  ...
  try {
    ...
    const grammar = nearley.Grammar.fromCompiled(mara)
    grammar.start = 'main'
  ...
  for (const z of zoneArray) {
    ...
    Object.keys(z).map(k => {
      ...
      if ([ 'exchange', 'dname', 'cname', 'mname', 'rname', 'target' ].includes(k) && /%/.test(z[k])) {
        z[k] = rr.fullyQualify(z[k].replace(/%/g, zoneOpts.origin))
      }
    ...
    if (zoneOpts.verbose) console.dir(z, { depth: null })
    try {
