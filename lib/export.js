...
const isBlank = (rr) => typeof rr === 'string' && rr.trim() === ''
...
const isDirective = (rr) => rr && !rr.toBind && (rr.$ORIGIN !== undefined || rr.$TTL !== undefined)
...
export function toBind(rrs, opts = {}) {
  ...
  if (opts.ttl !== undefined && !has('$TTL')) out.push(`$TTL ${opts.ttl}`)
...
export function toJSON(rrs) {
  ...
  for (const rr of rrs) {
    ...
    // A comment is presentation, not data. Drop it from the output without
    // deleting it from the caller's record, which these are now exported to.
    const { comment, ...fields } = rr.toJSON()
    void comment
