...
describe('bind', function () {
  describe('parseZoneFile', function () {
    ...
    it(`parses SOA`, async () => {
      const r = await bind.parseZoneFile(
        `example.com.   86400   IN  SOA ns1.example.com.    hostmaster.example.com. (${os.EOL}\t\t2021102100    ; serial${os.EOL}\t\t16384   ; refresh${os.EOL}\t\t2048     ; retry${os.EOL}\t\t604800    ; expiry${os.EOL}\t\t2560   ; minimum${os.EOL}\t\t) ${os.EOL}`,
      )
    ...
    it(`parses SOA one liner`, async () => {
      const r = await bind.parseZoneFile(
        `example.com. 86400 IN SOA ns1.example.com. hostmaster.example.com. 2021102100 16384 2048 604800 2560`,
      )
      assert.deepStrictEqual(
        r[0],
        new RR.SOA({
    ...
    it('parses SOA line', async () => {
      ...
      bind.zoneOpts.ttl = 86400
      const r = await bind.parseZoneFile(
        `example.com.  IN  SOA ns1.example.com. matt.example.com. 1 7200 3600 1209600 3600`,
      )
      assert.deepStrictEqual(
        r[0],
        new RR.SOA({
